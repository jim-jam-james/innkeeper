...
- [ ] Build & publish to PyPI; verify `uvx innkeeper --vault /path`. *(deferred)*
- [x] README: hook → **architecture diagram** → quickstart + **copy-paste host-config snippet** → concepts (ontology, two-tier links, validate) → **ports & adapters** → tool/resource/prompt reference → roadmap.
- [x] `innkeeper init --vault PATH` CLI so the quickstart doesn't need a `python -c` one-liner.
- [x] Polish `examples/sample_vault/` as both demo and fixtures (generated by `build_sample_vault.py`).
- [x] MIT `LICENSE` file.
